...
import { MemberSince } from "../../util/date/index.js";
import useFollow from "../../hooks/useFollow.jsx";
import LoadingSpinner from "../../components/common/LoadingSpinner.jsx";
import toast from "react-hot-toast";
import { useUpdatedProfile } from "../../hooks/useUpdatedProfile.jsx";
const ProfilePage = () => {
	...
	const { updateProfile, isUpdating } = useUpdatedProfile();
	...
	const handleImgChange = (e, state) => {
		...
		if (file) {
			...
			reader.onload = () => {
				state === "banner" && setBanner(reader.result);
							...
							<div className='flex justify-end px-4 mt-5'>
								...
								{(banner || profilePic) && (
									<button
										...
										onClick={async () => {
											await updateProfile({banner, profilePic})
											setBanner(null);
											setProfilePic(null);
										}}
									>
							...
							<div className='flex flex-col gap-4 mt-14 px-4'>
								...
								<div className='flex gap-2 flex-wrap'>
									...
									<div className='flex gap-2 items-center'>
										...
										<span className='text-sm text-slate-500'>{MemberSince(user.createdAt)}</span>
									</div>
								</div>
								<div className='flex gap-2'>
									<div className='flex gap-1 items-center'>
										<span className='font-bold text-xs'>{user?.following.length}</span>
										<span className='text-slate-500 text-xs'><a href={`/profile/${username}/following`}>Following</a></span>
									</div>
									<div className='flex gap-1 items-center'>
										<span className='font-bold text-xs'>{user?.followers.length}</span>
										<span className='text-slate-500 text-xs'><a href={`/profile/${username}/followers`}>Followers</a></span>
									</div>
								</div>
							</div>
